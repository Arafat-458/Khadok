...
import { loadProducts, loadProductsSync, saveProducts } from './productCatalog';
import { db } from '../firebase.init';

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';

const Products = () => {
  const [products, setProducts] = useState(() => loadProductsSync());
  ...
  useEffect(() => {
    ...
    const syncProducts = async () => {
      ...
      setProducts(data.map((product) => ({
        ...product,
        reviewSummary: summariesByProduct.get(String(product.id)) || null
      })));
    ...
    const unsubscribe = onSnapshot(collection(db, 'products'), (snapshot) => {
      // The snapshot already contains the fresh Firestore data; avoid doing a
      // second full Firestore query every time it changes.
      const remoteProducts = snapshot.docs.map((item) => ({ id: item.id, ...item.data() }));
      if (!remoteProducts.length || !active) return;
      const merged = new Map(loadProductsSync().map((product) => [String(product.id), product]));
      remoteProducts.forEach((product) => merged.set(String(product.id), { ...merged.get(String(product.id)), ...product }));
      const nextProducts = saveProducts(Array.from(merged.values()));
